...
namespace Binus.SampleWebAPI.WebAPI
{
    ...
    using Data.Infrastructures.Serpong.MSSQL;
    using Data.Infrastructures.Serpong.MongoDB;
    using Data.Infrastructures.Serpong.Oracle;
    using Data.DBContext.Serpong.MSSQL;
    using App_Start.Routing;
    using App_Start.JWT.LDAP;
    using MongoDB.Bson.Serialization;
    using MongoDB.Bson.Serialization.Serializers;
    using Data.Repositories.Serpong.MSSQL.Backend;
    using Data.Infrastructures.Common.MSSQL;
    using App_Start.JWT.BookDB;
    using Services.Serpong.BookDBAPP.MSSQL.Backend;
    ...
    public partial class Startup
    {
        public void Configuration(IAppBuilder Builder)
        {
            ...
            Configuration.Filters.Add(new APPIDFilter());
            ...
            Container.RegisterType<BinusianIDNewIDBFactory, BinusianIDNewDBFactory>(new HierarchicalLifetimeManager());
            Container.RegisterType<HRISDBIDBFactory, HRISDBDBFactory>(new HierarchicalLifetimeManager());
            Container.RegisterType<BinusWebAPIIDBFactory, BinusWebAPIDBFactory>(new HierarchicalLifetimeManager());

           
            Container.RegisterType<BookDBOracleIDBFactory, BookDBOracleDBFactory>(new HierarchicalLifetimeManager());
            
            Container.RegisterTypes(
            AllClasses.FromAssemblies(typeof(msBookMSSQLRepository).Assembly)
            ...
            Container.RegisterTypes(
           AllClasses.FromAssemblies(typeof(msBookService).Assembly)
        ...
        private void ConfigureOAuth(IAppBuilder APP)
        {
            ...
            var OAuthServerOptions = new OAuthAuthorizationServerOptions()
            {
                ...
                TokenEndpointPath = new PathString("/oauth2/LDAP/token"),
                RefreshTokenProvider = new RefreshTokenProvider(),
                AccessTokenExpireTimeSpan = TimeSpan.FromMinutes(Convert.ToInt16(ConfigurationManager.AppSettings["TokenTime"])),
                Provider = new LDAPCustomOAuthProvider()
            };

            //Login To database BookDB
            var OAuthServerOptions2 = new OAuthAuthorizationServerOptions()
            {
                AllowInsecureHttp = true,
            ...
            APP.UseOAuthAuthorizationServer(OAuthServerOptions2);
