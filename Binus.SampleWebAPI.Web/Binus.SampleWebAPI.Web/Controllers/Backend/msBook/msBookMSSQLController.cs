...
namespace Binus.SampleWebAPI.Web.Controllers.Backend.msBook
{
    ...
    public class msBookMSSQLController : Controller
    {
        public ActionResult Index()
        {
            RESTResult Result = (new REST(Global.WebAPIBaseURL, "/api/Serpong/BookDB/v1/Backend/msBookMSSQL/Get", REST.Method.POST).Result);
            if(Result.Success)
            {
        ...
        public ActionResult Manipulation(msBookViewModel Data)
        {
            ...
            RESTResult ResultItem = (new REST(Global.WebAPIBaseURL, "/api/Serpong/BookDB/v1/Backend/msBookMSSQL/Manipulation", REST.Method.POST,Data.Book).Result);
            if (ResultItem.Success)
            {
                return Json(new { Result = "Success", URL = "/msBookMSSQL" });
            }
            else
            {
        ...
        {
            RESTResult Result = (new REST(Global.WebAPIBaseURL, "/api/Serpong/BookDB/v1/Backend/msBookMSSQL/Get", REST.Method.POST, Data).Result);
            if (Result.Success)
            {
